...
class DM2FNet_woPhy(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        ...
        self.down0 = nn.Sequential(
            nn.Conv2d(64, num_features, kernel_size=1), BReLU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), BReLU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), BReLU()
        )
        self.down1 = nn.Sequential(
            nn.Conv2d(256, num_features, kernel_size=1), BReLU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), BReLU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), BReLU()
        )
        self.down2 = nn.Sequential(
            nn.Conv2d(512, num_features, kernel_size=1), BReLU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), BReLU()
        )
        self.down3 = nn.Sequential(
            nn.Conv2d(1024, num_features, kernel_size=1), BReLU()
        )
        self.down4 = nn.Sequential(
            nn.Conv2d(2048, num_features, kernel_size=1), BReLU()
        )

        self.fuse3 = nn.Sequential(
            nn.Conv2d(num_features * 2, num_features, kernel_size=3, padding=1), BReLU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), BReLU(),
        ...
        self.p0 = nn.Sequential(
            nn.Conv2d(num_features, num_features // 2, kernel_size=3, padding=1), BReLU(),
            nn.Conv2d(num_features // 2, 3, kernel_size=1)
        )
        self.p1 = nn.Sequential(
            nn.Conv2d(num_features, num_features // 2, kernel_size=3, padding=1), BReLU(),
        ...
        self.attentional_fusion = nn.Sequential(
            nn.Conv2d(num_features, num_features // 2, kernel_size=3, padding=1), BReLU(),
            nn.Conv2d(num_features // 2, num_features // 2, kernel_size=3, padding=1), BReLU(),
        ...
        for m in self.modules():
            if isinstance(m, nn.SELU) or isinstance(m, nn.ReLU) or isinstance(m,BReLU):
                m.inplace = True
